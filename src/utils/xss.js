...
export const xssFilter = html => {
  ...
  const aTag = ['class', 'style', 'href', 'data-url', 'target', 'id']
  ...
  const brTag = ['style']
  const strongTag = ['style']
  const svgTag = ['svg', 'x', 'y', 'viewbox', 'width', 'height', 'style', 'g', 'line', 'xmlns', 'preserveaspectratio']
  const lineTag = ['style', 'x1', 'x2', 'y1', 'y2', 'fill', 'stroke', 'stroke-width', 'stroke-miterlimit']
  const gTag = ['style']
  const ulTag = ['style']
  const polygonTag = ['style', 'fill', 'points']
...
export const processLink = html => {
  return xss(html, {
    onTagAttr: function (tag, name, value) {
      ...
      if (tag === 'a' && name === 'href') {
        const val = xss.friendlyAttrValue(value)
