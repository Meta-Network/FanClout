      ...
      <div class="cardunit-r">
        ...
        <div class="cardunit-r-flows">
          ...
          <div class="cardunit-r-flows-like">
            <svg-icon
              :class="`${stats && 'like-touch'} ${flows.localLiked && 'active'}`"
              icon-class="mastodon-star"
              @click="likeClick"
            />
...
export default {
  ...
  computed: {
    ...
    username () {
      if (!this.card) return ''
      // return this.card.account.username + '@' + url.url(this.card.account.url).hostname
      return this.card.account.username
    },
    createTime () {
      if (!this.card) return ''
